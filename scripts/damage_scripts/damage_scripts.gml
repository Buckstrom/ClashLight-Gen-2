...
function damage_target(attack,combofactor,ethereal){
	if (is_undefined(ethereal)) {
		ethereal = 0
	}
	switch(attack.target) {
		default:
		damage_single(attack,combofactor,ethereal)
		break;
		case -2:
		damage_all(attack,combofactor,ethereal)
		break;
	}
}
...
function damage_single(attack, combofactor, ethereal){
	var _enemy = ds_list_find_value(targetRow, attack.target)
	if (is_undefined(_enemy)) {
		return;
	}
	_enemy.comboMultiplier = combofactor;
	takeDamage(_enemy, attack.damage,ethereal);
	++_enemy.comboCount;
}
function damage_all(attack, combofactor, ethereal){
...
function takeDamage(enemy, damage, ethereal) {
...
function calcCombo() {
	...
	if (_trackChange) {
		...
		for (var i = 0; i < _row; ++i) {
			...
			if (ds_map_exists(_enemy.debuffs, "lured") && _enemy.damageSum > 0 && _enemy.unluring) {
				if (useLureKB) {
